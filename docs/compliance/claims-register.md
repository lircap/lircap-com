...
- `allowed-direction` — acceptable as strategic direction, not a final legal/factual claim.
...
- `anonymise` — publish only anonymised/generalised version unless consent is documented.
- `exclude-until-cleared` — do not publish in pre-prod/client-facing copy.
- `safe-default` — approved default posture until sign-off.

| ID | Claim / copy area | Source | Risk class | Current status | Safe web posture / fallback | Owner / sign-off needed | Affected issues |
|---|---|---|---|---|---|---|---|
| CL-001 | “registered and authorised in Ireland (Company No. 801328)” / “Authorised” governance language | Sales deck slides 7 and 11 | Regulatory status | `exclude-until-cleared` / `needs-counsel` | State only that Harmonycove Limited trades as Lir Capital Partners and is registered with Company No. 801328. Do not use “authorised” unless authorising body and reference are confirmed. | Tim Cole / counsel | #8, #9, #12, #27 |
...
| CL-004 | Named Jagex / Carlyle / CVC / Haveli case-study narrative | Sales deck slide 9 | Named counterparty / confidentiality / substantiation | `anonymise` | Use anonymised proof unit unless written consent and public-source substantiation are on file. | Partners / counsel | #8, #9 |
...
| CL-011 | “TBC active deals” / track-record placeholders / “PLACEHOLDER - TO BE INSERTED” | Sales deck slides 1 and 10 | Placeholder / incomplete proof | `exclude-until-cleared` | Do not publish. Replace only with cleared, substantiated proof or remove section. | Partners / compliance | #8, #16, #27 |
